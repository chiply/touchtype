...
[![CI](https://github.com/chiply/touchtype/actions/workflows/ci.yml/badge.svg)](https://github.com/chiply/touchtype/actions/workflows/ci.yml)
...
```elisp
(use-package touchtype
  :straight (:host github :repo "chiply/touchtype"))
```
...
```elisp
(add-to-list 'load-path "/path/to/touchtype")
(require 'touchtype)
```
...
See [ALGORITHMS.md](ALGORITHMS.md) for detailed documentation of the
algorithms and data sources used by touchtype, including:

- **Bigram frequency table** — 26x26 letter transition weights from English corpus analysis
- **Pseudo-word generation** — weighted random walks over the bigram table
- **Confidence scoring** — combined speed and accuracy metric (derived from keybr.com)
- **Key unlock logic** — progressive mode's automatic key introduction
- **N-gram sources** — 100 common bigrams, trigrams, and tetragrams
- **Word list** — ~4,258 common English words
- **Narrative sources** — 20 Project Gutenberg classics
- **Code snippets** — 42 snippets across 8 languages
